...
public class ScannerInputService implements UserInputService {
	 ...
	 */
	@Override
	public String getVaildAnswer(String... strings) {
		while (true) {
	 ...
	 */
	@Override
	public int getIntFromLimit(int max, int min) {
		while (true) {
			int n = getInt();
			if (n >= min && n <= max) {
				return n;
	 ...
	 */
	@Override
	public int getIntFromLimit(int max, int min, boolean with) {
		while (true) {
			int n = getInt();
			if (n >= min && n <= max) {
				if (!with && n == min || n == max) {
	...
	@Override
	public int getIntFromLimit(int max, int min, boolean wmax, boolean wmin) {
		while (true) {
			int n = getInt();
			if (n >= min && n <= max) {
				if (!wmax && n == max) {
	 ...
	 */
	@Override
	public double getDoubleFromLimit(double max, double min) {
		while (true) {
			double n = getDouble();
			if (n >= min && n <= max) {
				return n;
	 ...
	 */
	@Override
	public double getDoubleFromLimit(double max, double min, boolean with) {
		while (true) {
			double n = getDouble();
			if (n >= min && n <= max) {
				if (!with && n == min || n == max) {
	...
	@Override
	public double getDoubleFromLimit(double max, double min, boolean wmax, boolean wmin) {
		while (true) {
			double n = getDouble();
			if (n >= min && n <= max) {
				if (!wmax && n == max) {
